# abd_substraction.py #
This script will calculate the proportion of each base at a given position for multiple sequences of the same length for two groups. It will then substract the base distribution of one group from the other. The output is an excel file containing line plots displaying the proportion for each base. 
